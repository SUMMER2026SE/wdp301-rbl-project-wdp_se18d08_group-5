...
class _SurveyScreenState extends ConsumerState<SurveyScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(surveyProvider, (previous, next) {
      ...
      } else if (next.status == SurveyStatus.invalid) {
        // Handle validation error
        if (context.mounted) {
          _formKey.currentState?.fields['username']?.invalidate(
            next.errorMessage ?? 'Validation failed',
          );
        }
      }
    });
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: surveyState.status == SurveyStatus.submitting
                          ? null
                          : () async {
                              // Simple: Just validate form and submit
                              if (_formKey.currentState?.saveAndValidate() ??
                                  false) {
                                final values = _formKey.currentState!.value;

                                // All validation logic is in the Notifier
                                await ref
                                    .read(surveyProvider.notifier)
                                    .submitWithValidation(values);
                              }
                            },
